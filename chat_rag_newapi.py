...
# # Logo settings
# LOGO_PATH = get_secret("LOGO_PATH", "assets/logo.png")  # local path
...
"""
Streamlit conversational RAG chat with debug-friendly logo and collapsible left panel.
Secrets order: st.secrets -> environment vars -> .env (python-dotenv).
Place optional logo at assets/logo.png or set LOGO_URL secret.
"""
...
# -----------------------
# Secrets helper (why: allow Streamlit Cloud + local dev)
# -----------------------
def get_secret(key: str, fallback: Optional[str] = None) -> Optional[str]:
    try:
        if key in st.secrets:
...
# -----------------------
# Init API clients (cached)
# -----------------------
if "openai_client" not in st.session_state:
    try:
        from openai import OpenAI
    except Exception:
        st.error("Install OpenAI client: pip install --upgrade openai")
        st.stop()
    st.session_state.openai_client = OpenAI(api_key=OPENAI_API_KEY)
...
# -----------------------
# Core helpers (why: encapsulate retrieval + chat)
# -----------------------
def get_query_embedding(text: str, model: str = EMBED_MODEL) -> List[float]:
    resp = openai_client.embeddings.create(model=model, input=text)
    return resp.data[0].embedding
...
def build_prompt_messages(question: str, retrieved: List[Dict[str, Any]], chat_history: List[Dict[str,str]]) -> List[Dict[str,str]]:
    # CardGuru persona; keep concise but explicit (why: consistent behaviour)
    system_msg = {
        "role": "system",
        "content": (
            "You are CardGuru, a domain-expert assistant for Indian credit cards.\n\n"
...
# -----------------------
# UI: debug-friendly logo + collapsible left panel
# -----------------------
st.set_page_config(page_title="CardGuru — RAG Chat", layout="wide")

def show_logo(width: int = 110) -> bool:
    """
    Show logo; prints diagnostics (why: help debug missing logo on Streamlit).
    Returns True if an image was displayed.
    """
    try:
        if LOGO_URL:
            st.write(f"Trying LOGO_URL: {LOGO_URL}")
            try:
                st.image(LOGO_URL, width=width)
                return True
            except Exception as e:
                st.warning(f"Failed to load LOGO_URL: {e}")
    except Exception:
        pass
    try:
        p = Path(LOGO_PATH)
        st.write(f"Trying local logo path: {p}")
        exists = p.exists()
        st.write(f"Exists: {exists}")
        if exists:
            try:
                size = p.stat().st_size
                st.write(f"File size: {size} bytes, suffix: {p.suffix}")
            except Exception:
                pass
            try:
                st.image(str(p), width=width)
                ...
                st.warning(f"Failed to load local image: {e}")
        else:
            st.info("Logo file not found at the configured local path.")
    except Exception as e:
        st.warning(f"Logo check error: {e}")
...
with st.sidebar:
    st.markdown("### Settings")
    show_left = st.checkbox("Show left panel (logo & info)", value=True)
    top_k = st.number_input("Top K (retrieval)", value=TOP_K_DEFAULT, min_value=1, max_value=20, step=1)
    model_sel = st.text_input("Chat model", value=CHAT_MODEL)
    embed_model_sel = st.text_input("Embedding model", value=EMBED_MODEL)
    if st.button("Clear conversation"):
...
if user_input:
    ...
    with st.spinner("Generating answer..."):
        try:
            ...
            st.error(f"OpenAI error: {e}")
            assistant_text = "Error: OpenAI API call failed."
    # Keep sources internally but do NOT show them in UI
    sources_meta = [{"source": r["metadata"].get("source", r["id"]), "score": r["score"], "snippet": r["snippet"]} for r in retrieved_items]
    st.session_state.messages.append({"role": "assistant", "content": assistant_text, "sources": sources_meta})

# Render messages (no sources displayed)
for msg in st.session_state.messages:
    role = msg.get("role")
    content = msg.get("content")
    if role == "user":
